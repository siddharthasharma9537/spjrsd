...
| `bookings` | Bookings / Counter Sale | view, create, edit, delete, reconcile (edit = Confirm/Complete a booking; reconcile = the end-of-day counter-sales totals view — Cashier-specific, distinct from just viewing individual bookings). Cancelling is *not* covered by `bookings:edit` - it's kept to EO/SysAdmin (`is_superuser`) only, even for a role granted `bookings:edit` for Confirm/Complete. Every other account can only *request* a cancellation with a reason; an EO/SysAdmin then approves (which actually cancels it) or rejects it via the Cancellation Requests screen - anyone with `bookings:edit` can see that queue and reject an obviously bad request, but only EO/SysAdmin can approve one. The Scan Ticket screen (`POST /admin/bookings/scan-complete`, `/scan-redeem-prasadam`) also runs on `bookings:edit` - scanning a ticket's QR code at the seva location or Prasadam Counter drives the same Completed transition and prasadam-redemption flag a manual click would, just triggered by a handheld scanner instead of a button. |
