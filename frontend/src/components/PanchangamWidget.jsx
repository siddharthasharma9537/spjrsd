...
import { Sun, Sunrise, Sunset, ChevronRight, Moon, CircleDot } from 'lucide-react';
...
export default function PanchangamWidget() {
  ...
  const rows = [
    { label: t('Tithi', 'తిథి'), value: t(data.tithi, data.tithi_telugu), timing: data.tithi_timing },
    { label: t('Nakshatra', 'నక్షత్రం'), value: t(data.nakshatra, data.nakshatra_telugu), timing: data.nakshatra_timing },
  ...
  return (
    <div className="bg-white border border-[#E6DCCA] rounded-xl p-5" data-testid="panchangam-widget">
      ...
      {(data.sunrise || data.sunset) && (
        <div className="flex items-center gap-4 pb-3 pt-3 border-t border-[#E6DCCA] text-xs text-[#5D4037]">
      ...
      {transitionCards.length > 0 && (
        <div className="pt-3 border-t border-[#E6DCCA] space-y-2.5" data-testid="upcoming-tithi-transitions">
          {transitionCards.map(c => (
            <div key={c.key} className="flex items-start gap-2">
              ...
              <div className="text-xs text-[#5D4037] min-w-0">
                <span className="font-medium text-[#2D1B0E]">{c.label}</span>
                {c.start_date && (
